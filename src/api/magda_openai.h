...
public:
  ...
  bool GenerateDSL(const char *question, const char *system_prompt,
                   WDL_FastString &out_dsl, WDL_FastString &error_msg);

  // Generate DSL code with REAPER state context
  bool GenerateDSLWithState(const char *question, const char *system_prompt,
  ...
  using StreamCallback =
      std::function<bool(const char *partial_dsl, bool is_done)>;

  // Generate DSL with streaming (for UI updates)
  bool GenerateDSLStream(const char *question, const char *system_prompt,
  ...
  bool GenerateJSFXStream(const char *question, const char *existing_code,
...
private:
  ...
  char *BuildRequestJSON(const char *question, const char *system_prompt,
                         const char *state_json);

  // Extract DSL from response JSON
  bool ExtractDSLFromResponse(const char *response_json, int response_len,
                              WDL_FastString &out_dsl,
                              WDL_FastString &error_msg);

  // Make HTTPS POST request
  bool SendHTTPSRequest(const char *url, const char *post_data,
