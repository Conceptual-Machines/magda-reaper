...
bool Interpreter::ParseTrackStatement(Tokenizer& tok) {
    ...
    if (params.Has("id")) {
        ...
        // Get index
        double (*GetMediaTrackInfo_Value)(MediaTrack*, const char*) = (double (*)(MediaTrack*, const char*))g_rec->GetFunc("GetMediaTrackInfo_Value");
        if (GetMediaTrackInfo_Value) {
            m_ctx.current_track_idx = (int)GetMediaTrackInfo_Value(m_ctx.current_track, "IP_TRACKNUMBER") - 1;
        }
    } else {
...
MediaTrack* Interpreter::CreateTrack(const Params& params) {
    ...
    bool (*GetSetMediaTrackInfo_String)(MediaTrack*, const char*, char*, bool) = (bool (*)(MediaTrack*, const char*, char*, bool))g_rec->GetFunc("GetSetMediaTrackInfo_String");
    int (*TrackFX_AddByName)(MediaTrack*, const char*, bool, int) = (int (*)(MediaTrack*, const char*, bool, int))g_rec->GetFunc("TrackFX_AddByName");
...
MediaTrack* Interpreter::GetTrackByName(const std::string& name) {
    ...
    if (!GetNumTracks || !GetTrack || !GetSetMediaTrackInfo_String) return nullptr;
...
MediaTrack* Interpreter::GetSelectedTrack() {
    ...
    MediaTrack* (*GetSelectedTrack2)(void*, int, bool) = (MediaTrack* (*)(void*, int, bool))g_rec->GetFunc("GetSelectedTrack2");
    if (!GetSelectedTrack2) return nullptr;

    return GetSelectedTrack2(nullptr, 0, false);
}

void Interpreter::DeleteTrack(MediaTrack* track) {
    if (!g_rec || !track) return;
...
void Interpreter::SetTrackProperties(MediaTrack* track, const Params& params) {
    ...
    bool (*GetSetMediaTrackInfo_String)(MediaTrack*, const char*, char*, bool) = (bool (*)(MediaTrack*, const char*, char*, bool))g_rec->GetFunc("GetSetMediaTrackInfo_String");
    bool (*SetMediaTrackInfo_Value)(MediaTrack*, const char*, double) = (bool (*)(MediaTrack*, const char*, double))g_rec->GetFunc("SetMediaTrackInfo_Value");
...
int Interpreter::GetProjectBPM() const {
    ...
    double (*GetProjectTimeSignature2)(void*, double*) = (double (*)(void*, double*))g_rec->GetFunc("GetProjectTimeSignature2");
    if (!GetProjectTimeSignature2) return 120;
...
double Interpreter::GetProjectTimeSignature() const {
    ...
    double (*TimeMap_GetTimeSigAtTime)(void*, double, int*, int*, double*) = (double (*)(void*, double, int*, int*, double*))g_rec->GetFunc("TimeMap_GetTimeSigAtTime");
    if (!TimeMap_GetTimeSigAtTime) return 4.0;
...
MediaItem* Interpreter::CreateClipAtBar(MediaTrack* track, int bar, int length_bars) {
    if (!g_rec || !track) return nullptr;

    MediaItem* (*AddMediaItemToTrack)(MediaTrack*) = (MediaItem* (*)(MediaTrack*))g_rec->GetFunc("AddMediaItemToTrack");
    bool (*SetMediaItemPosition)(MediaItem*, double, bool) = (bool (*)(MediaItem*, double, bool))g_rec->GetFunc("SetMediaItemPosition");
    bool (*SetMediaItemLength)(MediaItem*, double, bool) = (bool (*)(MediaItem*, double, bool))g_rec->GetFunc("SetMediaItemLength");
    MediaItem_Take* (*AddTakeToMediaItem)(MediaItem*) = (MediaItem_Take* (*)(MediaItem*))g_rec->GetFunc("AddTakeToMediaItem");
    bool (*SetMediaItemTake_Source)(MediaItem_Take*, void*) = (bool (*)(MediaItem_Take*, void*))g_rec->GetFunc("SetMediaItemTake_Source");
    void* (*PCM_Source_CreateFromType)(const char*) = (void* (*)(const char*))g_rec->GetFunc("PCM_Source_CreateFromType");
...
void Interpreter::DeleteClip(MediaTrack* track, int clip_index) {
    ...
    int (*GetTrackNumMediaItems)(MediaTrack*) = (int (*)(MediaTrack*))g_rec->GetFunc("GetTrackNumMediaItems");
    MediaItem* (*GetTrackMediaItem)(MediaTrack*, int) = (MediaItem* (*)(MediaTrack*, int))g_rec->GetFunc("GetTrackMediaItem");
    bool (*DeleteTrackMediaItem)(MediaTrack*, MediaItem*) = (bool (*)(MediaTrack*, MediaItem*))g_rec->GetFunc("DeleteTrackMediaItem");

    if (!GetTrackNumMediaItems || !GetTrackMediaItem || !DeleteTrackMediaItem) return;
...
bool Interpreter::AddAutomation(MediaTrack* track, const Params& params) {
    ...
    // 2. Generate curve points based on curve type (fade_in, fade_out, sine, etc.)
...
bool Interpreter::FilterTracks(const std::string& field, const std::string& op, const std::string& value) {
    ...
    if (!GetNumTracks || !GetTrack || !GetSetMediaTrackInfo_String) return false;
