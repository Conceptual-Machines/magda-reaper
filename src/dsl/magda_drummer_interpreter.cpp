...
bool Interpreter::ExecutePattern(const char* params) {
    ...
    while (pos < paramStr.length()) {
        ...
        if (key == "drum") drumName = value;
        else if (key == "grid") grid = value;
        else if (key == "velocity") velocity = atoi(value.c_str());
    }
    ...
    bool success = MagdaActions::AddDrumPattern(trackIndex, drumName.c_str(),
                                                 grid.c_str(), velocity, nullptr, errorMsg);
