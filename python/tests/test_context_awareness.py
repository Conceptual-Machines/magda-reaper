...
class TestContextAwareness:
    ...
    @patch("magda.agents.operation_identifier.OperationIdentifier.identify_operations_with_llm")
    def test_track_creation_and_reference(self, mock_identify, pipeline):
        ...
        mock_identify.side_effect = [
            [{"type": "track", "description": "Create a new track called Lead Guitar", "parameters": {"name": "Lead Guitar"}}],
            [{"type": "effect", "description": "Add reverb to the Lead Guitar track", "parameters": {"track_name": "Lead Guitar", "effect": "reverb"}}]
        ]
    ...
    def test_multiple_tracks_and_effects(self, mock_identify, pipeline):
        ...
        mock_identify.side_effect = [
            [
                {"type": "track", "description": "Create a track called Bass", "parameters": {"name": "Bass"}},
                {"type": "track", "description": "Create a track called Drums", "parameters": {"name": "Drums"}}
            ],
            [
                {"type": "effect", "description": "Add reverb to Bass", "parameters": {"track_name": "Bass", "effect": "reverb"}},
                {"type": "effect", "description": "Add delay to Drums", "parameters": {"track_name": "Drums", "effect": "delay"}}
            ]
        ]
        ...
        with patch.object(pipeline.agents["track"], "execute") as mock_track:
            mock_track.side_effect = [
                {"daw_command": "track(Bass)", "result": {"id": "track_1", "name": "Bass"}},
                {"daw_command": "track(Drums)", "result": {"id": "track_2", "name": "Drums"}}
            ]
            
            # Mock effect agent responses
            with patch.object(pipeline.agents["effect"], "execute") as mock_effect:
                mock_effect.side_effect = [
                    {"daw_command": "effect(Bass, reverb)", "result": {"id": "effect_1", "track_id": "track_1", "type": "reverb"}},
                    {"daw_command": "effect(Drums, delay)", "result": {"id": "effect_2", "track_id": "track_2", "type": "delay"}}
                ]
    ...
    def test_volume_automation_context(self, mock_identify, pipeline):
        ...
        # Mock operation identification
        mock_identify.side_effect = [
            [{"type": "track", "description": "Create a track called Piano", "parameters": {"name": "Piano"}}],
            [{"type": "volume", "description": "Set the volume of Piano to -3dB", "parameters": {"track_name": "Piano", "volume": -3}}]
        ]
    ...
    def test_complex_workflow(self, mock_identify, pipeline):
        ...
        # Mock operation identification
        mock_identify.side_effect = [
            [{"type": "track", "description": "Create a track called Synth with Serum", "parameters": {"name": "Synth", "vst": "serum"}}],
            [
                {"type": "effect", "description": "Add reverb to Synth", "parameters": {"track_name": "Synth", "effect": "reverb"}},
                {"type": "effect", "description": "Add delay to Synth", "parameters": {"track_name": "Synth", "effect": "delay"}}
            ],
            [{"type": "volume", "description": "Set Synth volume to -6dB", "parameters": {"track_name": "Synth", "volume": -6}}],
            [{"type": "midi", "description": "Add C major chord to Synth", "parameters": {"track_name": "Synth", "chord": "C major"}}]
        ]
        ...
        with patch.object(pipeline.agents["track"], "execute") as mock_track:
            ...
            with patch.object(pipeline.agents["effect"], "execute") as mock_effect:
                mock_effect.side_effect = [
                    {"daw_command": "effect(Synth, reverb)", "result": {"id": "effect_1", "track_id": "track_1", "type": "reverb"}},
                    {"daw_command": "effect(Synth, delay)", "result": {"id": "effect_2", "track_id": "track_1", "type": "delay"}}
                ]
                ...
                with patch.object(pipeline.agents["volume"], "execute") as mock_volume:
                    mock_volume.return_value = {
                        ...
                        "result": {"id": "volume_1", "track_id": "track_1", "volume": -6}
                    }
                    ...
                    with patch.object(pipeline.agents["midi"], "execute") as mock_midi:
                        mock_midi.return_value = {
                            ...
                            "result": {"id": "midi_1", "track_id": "track_1", "chord": "C major"}
                        }
    ...
    def test_context_persistence(self, mock_identify, pipeline):
        ...
        # Mock operation identification
        mock_identify.side_effect = [
            [{"type": "track", "description": "Create a track called Guitar", "parameters": {"name": "Guitar"}}],
            [{"type": "effect", "description": "Add reverb to Guitar", "parameters": {"track_name": "Guitar", "effect": "reverb"}}],
            [{"type": "volume", "description": "Set Guitar volume to -3dB", "parameters": {"track_name": "Guitar", "volume": -3}}]
        ]
    ...
    def test_track_finding_methods(self, mock_identify, pipeline):
        ...
        # Mock operation identification
        mock_identify.return_value = [{"type": "track", "description": "Create a track called Test Track", "parameters": {"name": "Test Track"}}]
        
        # Mock track agent response
        with patch.object(pipeline.agents["track"], "execute") as mock_track:
            mock_track.return_value = {
                "daw_command": "track(Test Track)",
                "result": {"id": "track_1", "name": "Test Track", "magda_id": "track_obj_1_abc12345"}
            }
