...
class Operation(BaseModel):
    ...
    agent_name: str = Field(
        ..., description="Name of the agent to handle this operation"
    )
...
class EffectParameters(BaseModel):
    ...
    wet_mix: float = Field(
        0.5, ge=0.0, le=1.0, description="Wet mix amount (0.0 to 1.0)"
    )
    dry_mix: float = Field(
        0.5, ge=0.0, le=1.0, description="Dry mix amount (0.0 to 1.0)"
    )
    threshold: float = Field(
        -24.0, ge=-60.0, le=0.0, description="Compressor threshold in dB"
    )
    ...
    decay: float = Field(
        2.0, ge=0.1, le=10.0, description="Reverb decay time in seconds"
    )
    feedback: float = Field(
        0.25, ge=0.0, le=0.99, description="Delay feedback amount (0.0 to 0.99)"
    )
    delay_time: float = Field(
        0.5, ge=0.001, le=5.0, description="Delay time in seconds"
    )
    frequency: float = Field(
        1000.0, ge=20.0, le=20000.0, description="EQ frequency in Hz"
    )
...
class MIDIResult(BaseModel):
    ...
    transpose_semitones: int | None = Field(
