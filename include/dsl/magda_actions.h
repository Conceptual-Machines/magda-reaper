...
public:
  ...
  static bool ExecuteActions(const char *json, WDL_FastString &result, WDL_FastString &error_msg);

  // Public MIDI helpers - for use by arranger/drummer interpreters
  static bool AddMIDI(int track_index, wdl_json_element *notes_array, const char *take_name,
                      WDL_FastString &error_msg);

  // Drum pattern handler - converts grid notation to MIDI notes
  static bool AddDrumPattern(int track_index, const char *drum_name, const char *grid, int velocity,
                             const char *plugin_key, WDL_FastString &error_msg);

  // JSFX helper - saves code to file and optionally adds to track
  static bool SaveAndApplyJSFX(const char *jsfx_code, const char *effect_name, int track_index,
                               WDL_FastString &error_msg);
...
private:
  ...
  static bool SetTrackSelected(int track_index, bool selected, WDL_FastString &error_msg);
  ...
  static bool DeleteClip(int track_index, int clip_index, WDL_FastString &error_msg);
  ...
  static bool SetTrackProperties(int track_index, const char *name, const char *volume_db_str,
                                 const char *pan_str, const char *mute_str, const char *solo_str,
                                 const char *selected_str, const char *color_str,
                                 WDL_FastString &error_msg);
  static bool SetClipProperties(int track_index, const char *clip_str, const char *position_str,
