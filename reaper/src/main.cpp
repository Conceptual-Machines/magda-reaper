...
void magdaAction(int command_id, int flag) {
    void (*ShowConsoleMsg)(const char* msg) = (void (*)(const char*))g_rec->GetFunc("ShowConsoleMsg");
...
void menuHook(const char* menuidstr, void* menu, int flag) {
    ...
    if (menuidstr && strcmp(menuidstr, "Main extensions") == 0 && flag == 0) {
        ...
        // Other extensions might add items during the same hook call, so we check right before inserting
...
extern "C" {
        ...
        reaper_plugin_info_t *rec)
    {
        ...
        hookcommand2_t hookcmd = [](KbdSectionInfo* sec, int command, int val, int val2, int relmode, HWND hwnd) -> bool {
            if (command == MAGDA_MENU_CMD_ID || 
                command == MAGDA_CMD_OPEN || 
                command == MAGDA_CMD_SETTINGS || 
                command == MAGDA_CMD_ABOUT) {
